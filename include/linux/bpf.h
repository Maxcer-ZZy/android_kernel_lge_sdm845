...
struct bpf_map_ops {
	...
	/* spin_lock map ops */
	void (*map_spin_lock)(struct bpf_map *map, u64 flags);
	void (*map_spin_unlock)(struct bpf_map *map, u64 flags);
};
...
/* BPF link - generic program attachment mechanism */
struct bpf_link {
	u32 id;
	u32 type;
...
/* BPF_LRU_MAP related structures */
struct bpf_lru_node {
	struct list_head list;
	u32 hash;
};
...
/* BPF link functions */
int bpf_link_get_fd_by_id(u32 id);
int bpf_link_get_next_id(u32 start_id, u32 *next_id);
